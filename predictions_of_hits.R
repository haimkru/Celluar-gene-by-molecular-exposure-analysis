###predictions_of_hits#####
#/oak/stanford/groups/smontgom/hkrupkin/GxE/Code/predictions_of_hits.R
#author: Haim Krupkin
#date: 03/21/2026
...
first_cell_type <- fread("/oak/stanford/groups/smontgom/hkrupkin/GxE/Code/average_expression_in_diffrent_cell_types.csv")
cibersort_wide  <- copy(first_cell_type)
setDT(cibersort_wide)
setnames(cibersort_wide, "b_cell_expression_exam1_standardized.tsv", "bcell")
GENE_NAME_COL <- "Name"
...
for (i in seq_along(CELL_CODES)) {
  ...
  # ✅ UPDATED: Added X_test and X_fit for downstream SHAP analysis
  locto_results[[test_cell]] <- list(
    cell_type   = test_cell, 
    model       = model, 
    feature_cols = FEATURE_COLS,
    X_test      = X_test,   # The matrix you will want to explain with SHAP
    X_fit       = X_fit,    # Useful if you need a background dataset for reference
    preds       = preds, 
    labels      = y_test,
    pr_curve    = pr_obj, 
    roc_curve   = roc_obj,
...
##saving the model for later explainability using shap :3 ###
model_output_path <- "/labs/smontgom/grps_smontgom/hkrupkin/GxE/plots/plots_for_figures/locto_xgboost_models_with_data.rds"
message("\n>>> Saving trained models and feature matrices for SHAP to: ", model_output_path)

# Save the comprehensive list object
saveRDS(locto_results, file = model_output_path)
...
saveRDS(summary_dt,"/labs/smontgom/grps_smontgom/hkrupkin/GxE/plots/plots_for_figures/lift_over_simple_modelsV2.rds")
summary_dt<-readRDS("/labs/smontgom/grps_smontgom/hkrupkin/GxE/plots/plots_for_figures/lift_over_simple_modelsV2.rds")
...
p4
ggsave(plot=p4,
       file="/labs/smontgom/grps_smontgom/hkrupkin/GxE/plots/plots_for_figures/lift_over_simple_modelsV2.png",
       width=3,
       height=3,
       dpi=300
